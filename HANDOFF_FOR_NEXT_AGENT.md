...
`app.js`에는 Places resolved DB를 읽어 지도 링크를 보강하는 참고 구현이 있습니다. 다만 현재 `index.html`에서 로드하지 않으므로, 실제 화면 수정을 할 때는 `index.html` 데이터를 먼저 고치세요. `app.js`의 `fetch("./data/places.resolved.json")`는 `file://`에서 막힐 수 있어 정적 서버 환경에서 확인해야 합니다.
...
- [https://honeymoon-delta.vercel.app/](https://honeymoon-delta.vercel.app/)

GitHub Pages는 `main` 브랜치 루트(`/`) 기반입니다.
Vercel은 같은 저장소를 `honeymoon` 프로젝트로 배포하며, `/api/*` Google Places 프록시 함수도 함께 실행합니다.
...
Vercel 쪽 Places API 점검:

1. Vercel 프로젝트 `honeymoon` 확인
2. Production URL `https://honeymoon-delta.vercel.app`
3. `GOOGLE_PLACES_API_KEY` 환경변수 등록 여부 확인
4. `/api/google-place-search?query=Nicolau%20Lisboa` 호출 확인
